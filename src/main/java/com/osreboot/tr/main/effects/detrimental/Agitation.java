...
public class Agitation extends NodeEffects{
	...
	public void tick(DataTable d){
		...
		if(active.get(d.getPlayer().getName()) > 0){
			...
			d.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.FAST_DIGGING, 20, 1));
			if(new Random().nextInt((31 - d.nodes[14])*2) == 0) d.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 10, 1));
